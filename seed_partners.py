...
async def seed_database():
    ...
    try:
        # 1. Fetch existing names so re-running never duplicates or wipes other partners
        ...
        for partner in partners_data:
            if partner["company_name"].strip().lower() in existing_names:
                print(f"    SKIP (already exists): {partner['company_name']}")
                continue
